...
document.addEventListener('DOMContentLoaded', function() {
    ...
    saveButton.addEventListener('click', async () => {
        ...
        // Loop melalui semua nama equipment untuk mengumpulkan status dan keterangan
        equipmentNames.forEach(name => {
            const id = name.toLowerCase().replace(/[\s\(\)\.\-\/]/g, '');
            const status = formData.get(id);
            const keterangan = formData.get(`keterangan-${id}`);

            if(status && status !== 'NORMAL') { // Hanya simpan yang tidak normal atau yang diubah
                reportData.equipment_status[id] = { status: status };
                if (keterangan) {
                    reportData.equipment_status[id].keterangan = keterangan;
                }
            } else if (status === 'NORMAL' && keterangan) { // Kasus khusus jika normal tapi ada keterangan
                 reportData.equipment_status[id] = { status: status, keterangan: keterangan };
            }
        });

        // Ambil sisa data form
