...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const loadAndRenderReports = async () => {
        ...
        if (result.success && result.data.length > 0) {
            ...
            result.data.forEach(item => {
             ...
             listElement.innerHTML = `<li class="text-center p-10 text-gray-500">Belum ada laporan TFP.</li>`;
        }
    };
    ...
    reportForm.addEventListener('submit', async (e) => {
        ...
        let result;
        if (reportData.id) {
            result = await window.api.updateTfpReport(reportData);
        } else {
            result = await window.api.saveTfpReport(reportData);
        }
