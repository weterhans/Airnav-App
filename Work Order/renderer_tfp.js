...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const autoFillPelaksana = async () => {
        ...
        // Memanggil API khusus TFP
        const result = await window.api.getTfpTechniciansBySchedule({ tanggal: selectedDate, dinas: selectedShift });
    ...
    const renderWoTable = async () => {
        // Memanggil API khusus TFP
        const result = await window.api.getTfpWorkorders();
        if (!result.success) {
        ...
        if (workOrders.length === 0) {
            woListContainer.innerHTML = '<div class="bg-white p-6 rounded-lg shadow text-center text-gray-500">Belum ada work order TFP yang dibuat.</div>';
            return;
        }
        woListContainer.innerHTML = `
            <div class="bg-white rounded-lg shadow">
                <div class="p-6 border-b"><h3 class="text-lg font-semibold">Daftar Work Order TFP</h3></div>
    ...
    const saveWorkOrder = async () => {
        ...
        // Memanggil API khusus TFP
        const result = await window.api.saveTfpWorkorder(woData);
        if (result.success) {
            Swal.fire({
                title: 'Berhasil!',
                text: result.message,
    ...
    window.editWorkOrder = async (id) => {
        // Memanggil API khusus TFP
        const result = await window.api.getTfpWorkorderById(id);
        if (!result.success || !result.data) {
            Swal.fire({ title: 'Gagal!', text: 'Gagal mengambil data work order.', icon: 'error' });
            return;
        }
    ...
    window.deleteWorkOrder = (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                // Memanggil API khusus TFP
                const deleteResult = await window.api.deleteTfpWorkorder(id);
                if(deleteResult.success) {
                    Swal.fire('Dihapus!', 'Work order berhasil dihapus.', 'success');
                    showWoList();
                } else {
